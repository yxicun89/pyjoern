...
def display_accurate_summary(all_features):
    ...
    if all_features:
        ...
        # その他は単純合計（モジュール処理も含む）
        total_loops = sum(features.get('loop_statements', 0) for features in all_features.values())
        total_conditions = sum(features.get('conditional_statements', 0) for features in all_features.values())
        ...
        print(f"  total_connected_components: {total_connected} (論理積)")
        print(f"  total_loop_statements: {total_loops} (合計)")
        print(f"  total_conditional_statements: {total_conditions} (合計)")
        print(f"  total_cycles: {total_cycles} (合計)")
        print(f"  total_paths: {total_paths} (合計)")
        print(f"  total_cyclomatic_complexity: {total_complexity} (合計)")
